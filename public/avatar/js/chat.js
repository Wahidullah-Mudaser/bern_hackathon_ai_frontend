    ...
    peerConnection.ontrack = function (event) {
        ...
        if (event.track.kind === 'video') {
            ...
            videoElement.addEventListener('loadeddata', () => {
                console.log('Video data loaded, starting play...')
                videoElement.play()
            })
            ...
            videoElement.style.borderRadius = '0 20px 20px 0'
            ...
            videoElement.onplaying = () => {
                ...
                // Show the avatar controls container when video connects
                const avatarControlsContainer = document.getElementById('avatarControlsContainer');
                if (avatarControlsContainer) {
                    avatarControlsContainer.style.display = 'flex';
                    console.log('Avatar controls container shown in setupWebRTC');
                } else {
                    console.log('Avatar controls container not found in setupWebRTC');
                }
...
window.startSession = () => {
    ...
    // Show the avatar controls container
    const avatarControlsContainer = document.getElementById('avatarControlsContainer');
    if (avatarControlsContainer) {
        avatarControlsContainer.style.display = 'flex';
        console.log('Avatar controls container shown');
    } else {
        console.log('Avatar controls container not found');
    }
    
    // Update the header to "ACCESS VOYAGE"
    const mainHeader = document.getElementById('mainHeader');
    if (mainHeader) {
        mainHeader.textContent = 'ACCESS VOYAGE';
    }
    ...
    if (document.getElementById('useLocalVideoForIdle').checked) {
        ...
        // Show the avatar controls container immediately for local video mode
        const avatarControlsContainer = document.getElementById('avatarControlsContainer');
        if (avatarControlsContainer) {
            avatarControlsContainer.style.display = 'flex';
        }
        
        // Check the Type Message checkbox by default and send "Hi" automatically
        document.getElementById('showTypeMessage').checked = true
        window.updateTypeMessageBox()
        
        // Auto-send "Hi" message to start conversation
        setTimeout(() => {
            handleUserQuery("Hi", "Hi", "")
        }, 1000)
    ...
    // Check the Type Message checkbox by default when session starts
    setTimeout(() => {
        document.getElementById('showTypeMessage').checked = true
        window.updateTypeMessageBox()
        
        // Auto-send "Hi" message to start conversation after connection is established
        setTimeout(() => {
            handleUserQuery("Hi", "Hi", "")
        }, 2000)
    }, 500)
...
window.stopSession = () => {
    ...
    // Hide the avatar controls container
    const avatarControlsContainer = document.getElementById('avatarControlsContainer');
    if (avatarControlsContainer) {
        avatarControlsContainer.style.display = 'none';
    }
    
    // Revert the header back to original text when session ends
    const mainHeader = document.getElementById('mainHeader');
    if (mainHeader) {
        mainHeader.textContent = 'Talking Avatar Chat Demo';
    }
...
window.updateTypeMessageBox = () => {
    if (document.getElementById('showTypeMessage').checked) {
        ...
        document.getElementById('userMessageBox').hidden = true
        document.getElementById('uploadImgIcon').hidden = true
        // Also hide the entire chat input section when checkbox is disabled
    ...
    // Show the chat input section when checkbox is enabled
    if (document.getElementById('showTypeMessage').checked) {
        const chatInputSection = document.getElementById('chatInputSection');
        if (chatInputSection) {
...
// Add missing functions for chat interface
window.hideChatInterface = () => {
    // Hide the main content container initially (not the chat interface itself)
    const mainContentContainer = document.getElementById('mainContentContainer');
    if (mainContentContainer) {
        mainContentContainer.style.display = 'none';
    }
...
window.sendMessage = () => {
    ...
    if (userMessageBox) {
        const userQuery = userMessageBox.innerText;
...
window.insertSuggestion = (suggestion) => {
    const userMessageBox = document.getElementById('userMessageBox');
    if (userMessageBox) {
        userMessageBox.innerText = suggestion;
